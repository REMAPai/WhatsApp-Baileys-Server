...
async function init() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      const shouldReconnect = statusCode !== DisconnectReason.loggedOut && !manualDisconnect;
      ...
      if (shouldReconnect) {
        ...
        logger.error('Logged out. Delete the auth_info folder and restart to re-authenticate.');
      }
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    for (const msg of messages) {
      ...
      logger.info(
        { from: messageData.remoteJid, text: messageData.textContent, type: messageType },
        'Message received'
      );
